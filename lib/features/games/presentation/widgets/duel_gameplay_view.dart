...
class DuelGameplayView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return switch (state.phase) {
      ...
      GamePhase.paused || GamePhase.playing || GamePhase.roundResult => _buildPlaying(context),
        ...
        child: Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Icon(Icons.pause_circle_filled, size: 80, color: colorScheme.primary),
  ...
  Widget _buildPlaying(BuildContext context) {
    ...
    final isPaused = state.phase == GamePhase.paused;
      ...
      children: [
        Column(
          children: [
            ...
            Expanded(
              child: IgnorePointer(
                ignoring: isFinalResult || isPaused, // Disable taps when round is over or paused
                child: AnimatedOpacity(
                  opacity: (isFinalResult || isPaused) ? 0.6 : 1.0,
                  duration: const Duration(milliseconds: 300),
  ...
  Widget _buildReadZone(ColorScheme colorScheme) {
    return Padding(
      ...
      child: Row(
        children: [
          // Left: word card + result feedback
          Expanded(
            flex: 3,
            child: SingleChildScrollView(
              child: Column(
                ...
                const SizedBox(height: 16),
                // Highlighted result directly under the word
                  _buildHighlightedTranscript(colorScheme),
                ],
              ),
            ),
          ),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  // No longer show the simple text box as we have highlighted view now
                ],
              ),
            ),
          ),
  ...
  Widget _buildHighlightedTranscript(ColorScheme colorScheme) {
    if (state.recognizedWords.isEmpty) return const SizedBox.shrink();

    final targetWords = (state.targetWord ?? "").toLowerCase().trim().split(RegExp(r'\s+'));
    final recognizedWords = state.recognizedWords.toLowerCase().trim().split(RegExp(r'\s+'));
    ...
    return Wrap(
      ...
      children: recognizedWords.map((word) {
        final isCorrect = targetWords.contains(word);
        return Container(
